...
class AbstractMessageStorage(ABC, Generic[T]):
    ...
    async def load_chain(self, initial_message: MessagePair) -> MessagePair:
        def parse_ids(content: str) -> tuple[int, int]:
            channel_id, message_id = content.splitlines()[0].split(" ")
    ...
    @abstractmethod
    async def decode_content(self, content: str) -> T:
        """
        Receives the content of the messages joined by newlines.
        """
        pass

    async def _read_message(self, pair: MessagePair) -> str:
    ...
    async def read_all(self) -> T:
        ...
        return await self.decode_content("\n".join(contents))
    ...
    def _create_messages(
        ...
        while len(content) > 0:
            if len(current_message) + len(content[0]) + 1 <= 1900:
                current_message += content.pop(0) + "\n"
            else:
